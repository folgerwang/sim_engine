...
// Each vertex carries the index of the triangle it belongs to so the
// fragment shader can hash that into a flat per-triangle colour. The
// positions are already in world space (the FBX collision builder bakes
// node transforms into vertex_position_), so the push-constant transform
// is normally identity but kept for symmetry with cluster_debug.vert.
//
// Vertex layout fed by the C++ CollisionDebugDraw helper:
//   binding 0, location VINPUT_POSITION = 0 : vec3  position
//   binding 1, location 15                  : uint  triangle_id (one per
//                                                  vertex; all three
