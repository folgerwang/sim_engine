...
Categories:
- WALKABLE_SURFACE: walkable horizontal surfaces underfoot. ALL outdoor ground — roads, streets, asphalt, pavement, sidewalks, cobblestones, plazas, courtyards, paths — plus interior floors, floor tiles, and doormats. If a surface is ground you could stand on, it is WALKABLE_SURFACE, never CEILING.
- WALL: blocking vertical surfaces. Brick, plaster, stucco, exterior cladding, building facades, low barriers, railings, fences, roof shingles (treat roofs as Wall for blocking). IMPORTANT: plaster and stucco are ALWAYS walls, never WALKABLE_SURFACE. Concrete is WALKABLE_SURFACE only when the name says it is ground (pavement/road/sidewalk/floor-tile); a bare "Concrete" / "Concrete_White" / "Concrete_Plaster" material is building cladding → WALL.
